...
// 必须定义在主 IIFE 之前：httpRequest 会在 IIFE 同步执行阶段被调用，放到文件底部会触发 TDZ 报错
...
复制以上链接到 Safari 打开（替换为你的信息）`;
    ...
    const url = `${qlUrl}/open/auth/token?client_id=${encodeURIComponent(clientId)}&client_secret=${encodeURIComponent(clientSecret)}`;
...
// ============= HTTP 请求封装 =============
// 与 jd_cookie_sync.js 保持一致：Env 只暴露 get/post，非 GET 请求需把真实动词放进 opts.method
